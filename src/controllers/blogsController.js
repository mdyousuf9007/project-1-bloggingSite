  ...
  try {
    ...
    if (Object.keys(req.body).length != 0) {
      ...
      if (!body) {
        return res
          .status(400)
          .send({ status: false, msg: "Body field is required" });
      }
      if (!category) {
        return res
          .status(400)
          .send({ status: false, msg: "Categoryfield is required" });
      }
      if (!subcategory) {
        return res
          .status(400)
      ...
      return res
        .status(400)
        .send({ status: false, msg: "no data to create blog" });
    }
  } catch (error) {
  ...
  try {
    ...
    if (!mongoose.isValidObjectId(blogId))
      return res
        ...
        .send({
          Status: false,
          message: `Please enter valid blogId ${blogId} `,
        });
    ...
    if (req.validToken.authorId != findblog.authorId)
      return res
        .status(403)
        .send({ status: false, msg: "You are not Authorised to this task" });
...
const deleteBlog = async function (req, res) {
  try {
    ...
    if (!mongoose.isValidObjectId(data))
      return res
        .status(400)
        .send({ Status: false, message: `Please enter valid blogId ${data} ` });

    if (!data) {
      return res
        .status(403)
        .send({ status: false, msg: "blog id is not present in params" });
    }
    let find = await blogsModel.findOne({ _id: data });
    if (!find) {
      return res
        .status(404)
        .send({ status: false, msg: "blog does not exits" });
    }
    ...
    //.................middleware passes the control to handler....................
    if (req.validToken.authorId !== find.authorId.toString())
      return res
        .status(403)
        .send({ status: false, msg: "you are not authorised to this task" });
...
const deleteByQuery = async function (req, res) {
  try {
    ...
    if (req.validToken.authorId !== find.authorId.toString())
      return res
        .status(403)
        .send({ status: false, msg: "you are Not Authorised to task" });
    ...
    return res
      .status(200)
      .send({ status: true, msg: "user is successfully deleted", data: saved });
  } catch (error) {
    return res.status(500).send({ status: false, msg: error.message });
  }
};
